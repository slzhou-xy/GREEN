...
class TrajDataLoader:
    ...
    def _collate_func(self, data_df):
        ...
        road_traj_inputs = torch.zeros((bz, max_road_len + 1 - Config.prediction_length), dtype=torch.long)
        grid_traj_inputs = torch.zeros((bz, max_grid_len + 1 - Config.prediction_length), dtype=torch.long)

        grid_feature_inputs = torch.zeros((bz, max_grid_len + 1 - Config.prediction_length, 4), dtype=torch.long)

        grid_time_emb_inputs = torch.zeros((bz, max_grid_len + 1 - Config.prediction_length, Config.hidden_emb_dim), dtype=torch.float32)

        road_week_inputs = torch.zeros((bz, max_road_len + 1 - Config.prediction_length), dtype=torch.long)
        road_minute_inputs = torch.zeros((bz, max_road_len + 1 - Config.prediction_length), dtype=torch.long)

        road_type_inputs = torch.zeros((bz, max_road_len + 1 - Config.prediction_length), dtype=torch.long)
        travel_time = []
